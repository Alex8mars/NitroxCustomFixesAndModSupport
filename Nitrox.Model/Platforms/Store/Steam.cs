    ...
    public sealed class Steam : IGamePlatform
    {
        ...
        private static ProcessStartInfo CreateSteamGameStartInfo(string gameFilePath, string? steamExe, string args, int steamAppId, bool skipSteam, bool bigPictureMode = false)
        {
            ...
            if (!skipSteam)
            {
                ...
                // >>> BepInEx integration (via env vars on the Steam-launched process)
                BepInExIntegration.ApplyEnvironmentForStringDictionary(startInfo.EnvironmentVariables, gameDirectory);

                return startInfo;
            }

            // Start through game executable (for custom args / VR etc.)
            ...
            // >>> BepInEx integration when launching the game exe directly
