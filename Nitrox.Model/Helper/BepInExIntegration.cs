...
public static class BepInExIntegration
{
        ...
        Action<string, string> setValue)
    {
        ...
        // Doorstop bools accept 1/0 or TRUE/FALSE. Align with the upstream launch scripts
        // by sending the numeric form so Proton/Wine builds parse it consistently.
        ...
        // Ignore DOORSTOP_DISABLE in the host environment when we explicitly enable BepInEx.
        ...
        Func<string, string?> getValue)
    {
        ...
        if (configPath != null)
        {
            yield return new("DOORSTOP_CONFIG_FILE", configPath);
        }
    }
